...
def item():
    ...
    if form.validate_on_submit():
        if form.id.data:  # it's an update
            try:
                result = DB.update(
                    "UPDATE IS601_S_Items set name = %s, description = %s, stock = %s, cost = %s,image=%s WHERE id = %s",
                    form.name.data,
            ...
            try:
                result = DB.update(
                    """INSERT INTO IS601_S_Items (name, description, stock, cost, image) 
                VALUES (%s,%s,%s,%s,%s)""",
                    form.name.data,
                    form.description.data,
                    form.stock.data,
    ...
    if id:
        try:
            result = DB.selectOne(
                "SELECT id, name, description, stock, cost,image FROM IS601_S_Items WHERE id = %s",
                id,
            )
...
def items():
    ...
    try:
        result = DB.selectAll(
            "SELECT id, name, description, stock, cost,Visibility,image FROM IS601_S_Items LIMIT 25",
        )
...
def shop_list():
    ...
    try:
        result = DB.selectAll(
            "SELECT id, name, description, stock, cost, image FROM IS601_S_Items WHERE stock > 0 AND Visibility = 1 LIMIT 25",
        )
        if result.status and result.rows:
            rows = result.rows
    except Exception as e:
...
def cart():
    ...
    if id and user_id:
        if quantity > 0:
            ...
            try:
                result = DB.delete(
                    "DELETE FROM IS601_S_Cart where item_id = %s and user_id = %s",
                    id,
                    user_id,
                )
                if result.status:
    ...
    try:
        result = DB.selectAll(
            """SELECT c.id, item_id, name, c.quantity, (c.quantity * c.cost) as subtotal 
